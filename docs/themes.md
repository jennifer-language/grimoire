...
Two sets of colours sit outside a theme's palette, for the same reason the two
side columns do:

| | |
| - | - |
| `--gr-syn-*` | the code tokens: one pair tuned to read on both a warm parchment and a cool slate, which beats ten hand-tuned sets and is one place to fix |
| `--gr-adm-*` | the [admonitions](markdown.md#admonitions): a warning is amber in every book or it is not a warning. Each fill is its hue at low alpha, so it composes with whatever the theme put behind it |

A theme does not have to know either exists. To use different values, redefine
the properties in a `<style>` block in `html.footer`, which is emitted verbatim
and lands after the theme's stylesheet. Match the selectors the sheet uses: the
dark values sit under `:root[data-theme="dark"]` and inside a
`prefers-color-scheme` block, both of which outrank a bare `:root`, so an
override written only for `:root` applies in light mode alone.
